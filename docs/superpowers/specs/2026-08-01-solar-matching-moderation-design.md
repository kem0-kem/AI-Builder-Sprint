...
- 작성일: 2026-08-01
- 대상 서비스: SlowTalk FastAPI backend
- 상태: 사용자 승인 설계
- 관련 브랜치: `codex/slowtalk-backend-api`

## 1. 목적

SlowTalk의 단순 선착순 매칭을 두 단계 추천 방식으로 교체한다. 사용자의 첫 성공 매칭은 프로필의 관심사와 지역을 사용하고, 두 번째 성공 매칭부터는 Upstage Solar Embed 2 계열 임베딩으로 현재 편지와 후보자의 최근 편지 성향을 비교한다.
