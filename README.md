...
편지, 피드와 채팅 메시지를 전송하기 전에 앱이 `POST /api/v1/moderation/check`로 **비저장 사전검사**를 요청합니다. 백엔드는 Upstage Solar Chat에 원문 생성을 맡기는 대신 정해진 JSON 형식의 안전성 분류를 요청하고, 결과를 주의·개입·차단·긴급 UI로 변환합니다.

| 단계 | 분류 기준과 앱 동작 |
| --- | --- |
| **주의** | 낮은 심각도의 `REVIEW`. 표현을 다시 확인하도록 알리고 사용자가 수정하거나 그대로 전송할 수 있습니다. |
| **개입** | 그보다 높은 심각도의 `REVIEW`. 5초의 숙려 시간을 둔 뒤 수정하거나 전송할 수 있습니다. |
| **차단** | 유효 판정이 `BLOCK`. 그대로 전송하는 선택지를 제공하지 않고 수정만 허용합니다. |
| **긴급** | 자해 또는 폭력 범주이면서 심각도가 `CRITICAL`. 전송을 허용하지 않고 자살예방상담전화 **109**, 경찰 **112**, 구급·소방 **119** 도움 정보를 표시합니다. |

사전검사는 입력 내용을 마음잇기 데이터베이스의 `content_submissions`에 저장하거나 실제 작성 API를 대신 호출하지 않습니다. 단, 안전성 분류를 위해 입력 내용은 Upstage API로 전송됩니다. 또한 이 기능은 위험도를 조절하는 콘텐츠 단위 개입이며, 분류 결과만으로 계정을 자동 정지·탈퇴시키거나 사용자에게 제재 점수를 부여하지 않습니다.

현재 운영 백엔드의 `MODERATION_MODE`는 **`shadow`**입니다. 따라서 앱 사전검사와 단계별 안내는 동작하지만, 사용자가 최종 전송한 원래 작성 요청은 서버에서 모더레이션 결과 때문에 보류되거나 차단되지 않습니다. `enforce`로 전환한 환경에서만 원래 작성 API의 `REVIEW` 결과가 암호화 저장 후 `202 PENDING_REVIEW`로 보류되고, `BLOCK` 결과가 `422 CONTENT_POLICY_VIOLATION`으로 거절됩니다. 사전검사의 `operatorReviewRecommended` 표시는 검토 권고 신호이며, 사전검사 자체가 운영자 검토 건을 생성하는 것은 아닙니다.
