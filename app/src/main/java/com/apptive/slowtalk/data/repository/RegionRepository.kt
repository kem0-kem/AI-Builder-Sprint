...
class RegionRepository(private val api: RegionApi = RetrofitClient.regionApi) {
    ...
    suspend fun getProvinces(): Result<List<String>> {
        ...
        return try {
            Result.success(apiData { api.getProvinces() }.map { it.name })
        } catch (e: Exception) {
            Result.failure(e)
        }
    }
    ...
    suspend fun getDistricts(province: String): Result<List<String>> {
        ...
        return try {
            Result.success(apiData { api.getDistricts(province) }.map { it.name })
        } catch (e: Exception) {
            Result.failure(e)
        }
    }
    ...
    suspend fun getSubDistricts(province: String, district: String): Result<List<String>> {
        ...
        return try {
            Result.success(apiData { api.getSubDistricts(province, district) }.map { it.name })
        } catch (e: Exception) {
            Result.failure(e)
        }
    }
